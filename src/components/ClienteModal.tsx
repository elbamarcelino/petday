...
import { criarCliente, editarCliente } from '@/app/(dashboard)/dashboard/actions'
...
export function ClienteModal({ cliente, onClose }: Props) {
  ...
  const action = isEdit ? editarCliente.bind(null, cliente.id) : criarCliente
        ...
        <div className="bg-[var(--color-card)] rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
          ...
          <form action={formAction} className="space-y-3">
            <div>
              ...
              <input name="nome" required defaultValue={cliente?.nome} placeholder="Nome completo" className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">E-mail *</label>
              <input name="email" type="email" required defaultValue={cliente?.email} placeholder="email@exemplo.com" className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Telefone *</label>
              <input name="telefone" required defaultValue={cliente?.telefone} placeholder="(11) 99999-9999" className={inputCls} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Endereço</label>
              <input name="endereco" defaultValue={cliente?.endereco} placeholder="Rua, número, bairro" className={inputCls} />
            </div>
